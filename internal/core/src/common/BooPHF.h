...
#define MPHF_G4 0x3333333333333333ULL // 00110011 ... used to group the sum of 4 bits.
...
#define NBBUFF 10000
    ...
    public:
        ...
        template <class elem_t>
        std::pair<uint64_t, uint64_t> lookup1(elem_t elem)
        {
            if (!_built)
        ...
        std::vector<uint64_t> get_hash_array() const
        {
            return hash_array;
        }
        ...
        template <typename Iterator>
        void fillBuffer(std::vector<internal_hash_t> &buffer, std::shared_ptr<Iterator> shared_it, std::shared_ptr<Iterator> until_p,
        ...
        template <typename Iterator>
        void fillBufferCommon128(std::vector<internal_hash_t> &buffer, std::shared_ptr<Iterator> shared_it, std::shared_ptr<Iterator> until_p,
                                 uint64_t &inbuff, bool &isRunning)
        {
            auto until = *until_p;
            pthread_mutex_lock(&_mutex);
