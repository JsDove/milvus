...
PrimaryIndex::PrimaryIndex(const storage::FileManagerContext& ctx, bool is_load)
    {
    ...
    primary_index_ = std::make_unique<primaryIndex::PrimaryIndex<primaryIndex::StringHasher>>();
}
...
IndexStatsPtr
PrimaryIndex::Upload(const Config& config) {
    ...
    AssertInfo(disk_file_manager_->AddFile(index_file),
               "failed to add primary index file: {}", index_file);
    ...
    for (auto& file : remote_paths_to_size) {
        index_files.emplace_back(file.first,
                                 file.second);
    }
...
PrimaryIndex::Load(const Config& config) {
    ...
    AssertInfo(boost::filesystem::exists(index_file), 
               "primary index file not exist: {}", index_file);
...
int64_t
PrimaryIndex::query(std::string primary_key) {
    AssertInfo(is_built_, "PrimaryIndex has not been built yet");
